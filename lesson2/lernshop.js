...
class List {
    ...
    constructor(cartget) {
        let acceptitem = this.getitem() // получаем данные из бека (getitem) в виде массива
        acceptitem = acceptitem.map(item => { // map , массивы который получила разбивает на элементы и передае по одному
...
// список должен уметь пополнятся товарами (брать откуда-то данные)
    getitem() {
        return [
            {name: 'T_Shrt', price: 500, img: 'img/1.png' },
            {name: 'pants', price: 1000, img: 'img/2.png'},
            {name: 'shoes', price: 1500, img: 'img/3.png'},
        ]
 }
